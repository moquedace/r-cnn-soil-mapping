...
# ══════════════════════════════════════════════════════════════════════════════
# 06 — Spatial prediction 2D-tiled (block-streaming, seed ensemble)
#
# Versão 2D do script 05: cada worker recebe um RETÂNGULO do raster (fatia
# de linhas E colunas) em vez de apenas uma faixa de linhas. Isso reduz o
# custo de RAM por processo proporcional à fração de colunas do tile, o que
# permite aumentar max_concurrent e usar mais núcleos.
#
# Exemplo (global 160 k colunas, 187 bandas):
#   script 05 (1D): cada shard lê ~160 k cols -> ~240 MB/linha -> piso ~7-8 GB
#   script 06 (2D, n_col_shards=4): cada shard lê ~40 k cols -> ~60 MB/linha
#     -> piso ~2 GB -> max_concurrent pode subir para ~8 sem pressionar RAM
#
# Cada worker escreve um tile exatamente no seu retângulo (sem NA padding).
# 06b_merge_spatial_parts.R mosaica todos os tiles ao final.
#
# Compatibilidade: 06 e 05 produzem tiles independentes em pastas separadas
# (parts_2d vs parts) — não se misturam no merge.
#
# CLI: Rscript 06_predict_spatial.R <row_shard_id> <col_shard_id>
...
if (length(.cli_args) >= 5L) {
  max_concurrent <- as.integer(.cli_args[5])
}
...
# output_block_rows maior -> menos overhead de I/O vs script 05.
