...
# ══════════════════════════════════════════════════════════════════════════════
# 06a — Orquestrador de predição espacial com tiling 2D
#
...
#   script 05 (1D, n_col_shards=1): ~240 MB/linha -> piso ~7-8 GB -> max_concurrent=2
#   script 06 (2D, n_col_shards=4): ~60 MB/linha  -> piso ~2 GB   -> max_concurrent=8
#
...
# Ao final roda 06b_merge_spatial_parts.R para montar os mapas finais.
# ══════════════════════════════════════════════════════════════════════════════

# ── Configuração ───────────────────────────────────────────────────────────────

# Fatias de linha: equivalente ao n_shards do 05a. Mais fatias = processo vive
...
# duplicada em build_patches_multi, reteste (06a_test.R, linha 1, 4 col-shards)
# deu:
...
#    nao amostradas no teste), reavaliar via 06c_estimate_eta.R e cair para 3.
max_concurrent <- 4
...
merge_script  <- file.path(script_dir, "06b_merge_spatial_parts.R")
...
# ── Resolve config_id/target_label (mesma logica do 06_predict_spatial.R) ─────
...
# sanidade passarem (ver 06_predict_spatial.R). Se o processo morre antes
...
message("\nRodando merge (06b_merge_spatial_parts.R)...\n")
