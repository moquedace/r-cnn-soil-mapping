...
#   MQI  – Model Quality Index: composite = (CCC × NSE) / (MAE / mean(obs)).
#           Balances accuracy, bias, and variability in one number.
#           Higher is better; values above 1 indicate a model that
#           predicts better than the mean scaled by relative error.
...
#' @return A one-row tibble with columns: n, ccc, r2, mae, nse, rmse, mqi.
calc_metrics <- function(obs, pred) {
  ...
  tibble::tibble(n = length(obs), ccc = ccc_val, r2 = r2_val, mae = mae_val,
                 nse = nse_val, rmse = rmse_val, mqi = mqi_val)
}
